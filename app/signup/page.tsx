        ...
        <div className="relative w-[98vw] max-w-xs sm:max-w-sm md:max-w-md rounded-2xl border border-zinc-300 dark:border-zinc-800 bg-white dark:bg-zinc-900 shadow-[0_0_30px_rgba(99,102,241,0.4)] dark:shadow-[0_0_30px_rgba(99,102,241,0.3)] p-6">
          ...
          <form onSubmit={handleSignup} className="flex flex-col gap-4">
            ...
            <div className="relative">
              ...
              {!passwordVisible && (
                <div className="absolute top-1/2 -translate-y-1/2 w-1 h-5 bg-zinc-300 dark:bg-zinc-600 rounded pointer-events-none overflow-hidden" style={{ left: `${(passwordInputRef.current?.selectionStart ?? 0) * 9 + 16}px` }}>
                  <div className="absolute bottom-0 left-0 w-full bg-indigo-600 rounded" style={{ height: `${(password.length / MAX_PASSWORD_LENGTH) * 100}%` }}></div>
                </div>
              )}
              <button
                type="button"
              ...
              {validatePassword(password) && (
                <Check className="absolute right-16 top-1/2 -translate-y-1/2 text-green-500" size={20} />
              )}
            </div>
            <div className="relative">
              <input
              ...
              {!confirmPasswordVisible && (
                <div className="absolute top-1/2 -translate-y-1/2 w-1 h-5 bg-zinc-300 dark:bg-zinc-600 rounded pointer-events-none overflow-hidden" style={{ left: `${(confirmPasswordInputRef.current?.selectionStart ?? 0) * 9 + 16}px` }}>
                  <div className="absolute bottom-0 left-0 w-full bg-indigo-600 rounded" style={{ height: `${(confirmPassword.length / MAX_PASSWORD_LENGTH) * 100}%` }}></div>
                </div>
              )}
              <button
                type="button"
